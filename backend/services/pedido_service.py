...
def create_pedido(
    ...
    try:
        # Salvar arquivos localmente para validação e processamento
        ...
        # ── Validação antecipada (antes de criar registro no banco) ──────────
        order_items = parse_order_pdf(str(pedido_path))
        if not order_items:
            raise PedidoValidationError(
                "Nenhum item encontrado no PDF do pedido. "
                "Verifique se o arquivo é um PDF de pedido Bread King válido."
            )

        stock_data = parse_sales_report_pdf(str(relatorio_path))
        if not stock_data.get("items"):
            raise PedidoValidationError(
                "Nenhum item encontrado no PDF do relatório de vendas. "
                "Verifique se o arquivo é um relatório de vendas válido."
            )

        _, _, template_rows = load_template_rows(str(template_path))
        if not template_rows:
            raise PedidoValidationError(
                "Nenhuma linha de produto encontrada no template Excel. "
                "Verifique se o arquivo é um template válido."
            )
        # ─────────────────────────────────────────────────────────────────────

        # Criar registro no banco somente após validação bem-sucedida
